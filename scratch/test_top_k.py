...
# Test scaling only on top 5
for T in [1.5, 2.0, 2.5, 3.0]:
    print(f"\n--- TOP-5 ONLY TEMPERATURE SCALING T={T} ---")
    # Normalize top 5, then scale
    norm_top = top_probs / np.sum(top_probs)
    scaled_top = np.power(norm_top, 1 / T)
